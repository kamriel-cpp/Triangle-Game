...
{
	for (auto& shape : dungeon.shapes)
	{
		this->dungeon.push_back(sf::RectangleShape());
		this->dungeon.back().setSize(shape.getSize() * this->mapFactor);
		this->dungeon.back().setOrigin(this->dungeon.back().getSize() / 2.f);
		this->dungeon.back().setPosition(shape.getPosition() * this->mapFactor);
		if (shape.getSize() == dungeon.getRoomSize())
...
{
	this->player.setPosition(this->realPlayer->shape.getPosition() * this->mapFactor);
	this->player.setRotation(this->realPlayer->shape.getRotation());
}
